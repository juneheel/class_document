...
const changeLanguage = (sourceLanguage) => {
    if (sourceLanguage === targetLanguage) { // 원본 언어와 타겟 언어가 서로 같고,
        if (sourceLanguage === 'ko') { // 원본 언어가 한국어일 경우
            ...
            sourceSelect.value = targetLanguage;
...
const translateLanguage = async (sourceLanguage, text) => {
    ...
    .then(response => response.json())
    //.then(data => console.log(data)) object로고를 뽑은 뒤에 해당 번역값이 data.message.result.translatedText내부에있으므로 작성, notion 결과값 참조
    .then(data => {targetTextArea.textContent = data.message.result.translatedText})
    .catch(error => console.error(error));
}
